...
import { useWeatherSound } from "@/app/hooks/useWeatherSound";
...
export default function WeatherScreen() {
  ...
  // Pick values directly from API response based on settings
  const displayTemp = (c: number, f: number) =>
    Math.round(tempUnit === "F" ? f : c);
  const tempLabel = tempUnit === "F" ? "°F" : "°C";
  const windLabel = windUnit === "mph" ? "mph" : "km/h";
  const pressureLabel = pressureUnit === "in" ? "inHg" : "mb";
        ...
        >
          ...
          <View style={styles.tempRow}>
            ...
            <Text style={[styles.temp, { color: textColor }]}>
              {displayTemp(weather.tempC, weather.tempF)}
              {tempLabel}
            </Text>
          </View>
          ...
          <Text style={[styles.feelsLike, { color: subColor }]}>
            Feels like {displayTemp(weather.feelsLikeC, weather.feelsLikeF)}
            {tempLabel}
          </Text>
          ...
          <View style={styles.cardRow}>
            ...
            <WeatherCard
              icon="weather-windy"
              value={`${windUnit === "mph" ? weather.windMph : weather.windKph} ${windLabel}`}
          ...
          <View style={[styles.cardRow, { marginTop: 12 }]}>
            ...
            {weather.pressureMb != null && weather.pressureMb !== 0 && (
              <WeatherCard
                icon="gauge"
                value={`${pressureUnit === "in" ? weather.pressureIn : weather.pressureMb}`}
                label={pressureLabel}
                cardBg={cardBg}
                textColor={textColor}
                subColor={subColor}
          ...
          <View style={[styles.cardRow, { marginTop: 12 }]}>
            ...
            {weather.gustKph != null && weather.gustKph !== 0 && (
              <WeatherCard
                ...
                value={`${windUnit === "mph" ? weather.gustMph : weather.gustKph} ${windLabel}`}
                label="Gust"
                cardBg={cardBg}
                textColor={textColor}
                subColor={subColor}
